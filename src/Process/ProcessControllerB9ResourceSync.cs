...
{
	// B9PartSwitch can change a ProcessController resource field, but Kerbalism caches
	// the matching pseudo-resource after Configure(). Keep switchable processes in sync.
	public class ProcessControllerB9ResourceSync : PartModule
	{
		...
		private void SyncControllers()
		{
			foreach (ProcessController controller in part.FindModulesImplementing<ProcessController>())
			{
				...
				if (!lastResources.TryGetValue(controller, out string lastResource))
				{
