...
namespace KerbalismNative
{
	public class SystemHeatFissionReactorKerbalismUpdater : PartModule, IKerbalismModule
	{
		...
		public static string brokerTitle = Localizer.Format("#LOC_KerbalismBridge_Brokers_FissionReactor");
		...
		internal ModuleSystemHeatFissionReactor ReactorModule => reactorModule;
		...
		public virtual void Start()
		{
			if (Lib.IsFlight() || Lib.IsEditor())
			{
				...
				if (FirstLoad)
				{
					if (reactorModule != null)
					{
						MaxECGeneration = (float)reactorModule.ElectricalGeneration.Evaluate(100f);
		...
		public string PlannerUpdate(List<KeyValuePair<string, double>> resourceChangeRequest, CelestialBody body, Dictionary<string, double> environment)
		{
			if (reactorModule != null)
			{
				float curECGeneration = (float)reactorModule.ElectricalGeneration.Evaluate(reactorModule.CurrentReactorThrottle);
		...
		{
			ProtoPartModuleSnapshot reactor = BridgeUtils.FindPartModuleSnapshot(part_snapshot, reactorModuleName);
			if (reactor != null)
			{
