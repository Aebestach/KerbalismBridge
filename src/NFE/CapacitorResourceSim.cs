...
namespace KerbalismNFE
{
	...
	{
		internal static bool HasChargeHeadroom(DischargeCapacitor capacitor)
		{
			if (capacitor == null)
				return false;
			...
			if (stored == null)
				return capacitor.CurrentCharge < capacitor.MaximumCharge;

			float capacity = Mathf.Min((float)stored.maxAmount, capacitor.MaximumCharge);
			return stored.amount < capacity - 1e-4f;
		}

		/// <summary>Flight/background: charging only while there is room in StoredCharge.</summary>
		internal static bool IsCharging(DischargeCapacitor capacitor)
		{
			return capacitor != null
				&& capacitor.Enabled
				&& !capacitor.Discharging
				&& capacitor.ChargeRate > 0f
				&& HasChargeHeadroom(capacitor);
		}

		/// <summary>
		/// VAB/SPH: capacitors spawn full and NFE FixedUpdate is bypassed, so treat Enabled recharge as planner EC draw.
		/// Flight planner: same as <see cref="IsCharging"/>.
		/// </summary>
		internal static bool ShouldPlannerReportCharging(DischargeCapacitor capacitor)
		{
			if (capacitor == null || !capacitor.Enabled || capacitor.Discharging || capacitor.ChargeRate <= 0f)
				return false;
		...
		private static void UpdateCapacitorStatus(DischargeCapacitor capacitor)
		{
			...
			else if (capacitor.Enabled && !capacitor.Discharging && HasChargeHeadroom(capacitor))
			{
				capacitor.CapacitorStatus = KSP.Localization.Localizer.Format(
